$(document).ready(function() {
	...
	// 			height:'50%'
	// 		});
	// 		img[j].css({border:'15px solid #fff'});
	// 	}
	// 	photos.css({width:'100%'});
	// 	article.css({width:0,display:'none'});
	// })
	for(var i=0;i<oNum;i++){
		section[i].style.height=pageHeight+"px";
		aLi[i].index=i;
		aLi[i].onclick=function(){
			...
			sections.style.top=-pageHeight*this.index+"px";
	...
	EventUtil.addHandler(document,"mousewheel",scrollFunc);
	EventUtil.addHandler(document,"DOMMouseScroll",scrollFunc);
	function scrollFunc(event){
		...
		// 向下滚动，背景左移
		// 向上滚动,背景右移
		if(delta<0&&firstPosX>-10000||delta<0&&firstPosX==0||delta>0&&firstPosX==10000||delta>0&&firstPosX<0){
			if(delta<0){
				liNum++;
			...
			firstPosX=firstPosX+delta*500;
			secondPosX=secondPosX+delta*100;
			thirdPosX=thirdPosX+delta*300;
    ...
    getWheelDelta:function(event){
        if(event.wheelDelta){
            return (client.engine.opera && client.engine.opera<9.5?-event.wheelDelta/120:event.wheelDelta/120);
        }else{
